    ...
    return (
        <>
            <div className={`${styles.search_form} me-auto mb-2 mb-md-0`}>
                ...
                <button
                    className={`btn btn-lg btn-success ${styles.btn_nav} shadow px-3 py-2`}
                    type="button"
                    onClick={onSearch}
                >
                ...
                {searchResults.length > 0 && searchTerm.length > 0 &&
                    <ul className={`list-unstyled card shadow-sm p-2 ${styles.search_result}`}>
                        {searchResults.map((result) => (
                            <li key={result.id} className="d-flex my-2 p-2 card position-relative">
                                <Link href={`/product/${result.id}`} className="d-flex text-decoration-none">
                                    <img
                                        src={result.image}
                                        alt={result.name}
                                    ...
                                    <div className="w-75 mx-2 d-flex flex-column justify-content-between">
                                        ...
                                        <span className="text-muted">{toCurrency(result.price)}</span>
                ...
                {searchResults.length == 0 && searchTerm.length > 0 &&
                    <div className={`list-unstyled card shadow-sm p-2 fw-normal text-center text-muted ${styles.search_result}`}>
                        <h1>
                            No result
                        </h1>
                        <i>
                            Click search button when done typing.
                        </i>
                    </div>
                }
            </div>
